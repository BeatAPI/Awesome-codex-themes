...
const themeRoot = join(repoRoot, 'themes/night-city');

describe('night-city theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('night-city');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('lunar-phase');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"night-city\"]");
    expect(theme.css).toContain('--batch-theme-signature: "night-city"');
    expect(theme.css).toContain('--act-asset-lunar-phase');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('10-lucy.png');
    expect(notice).toContain('10-lucy-standard-codex-imagegen-v1.png');
  });
});
