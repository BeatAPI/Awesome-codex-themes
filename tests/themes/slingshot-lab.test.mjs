...
const themeRoot = join(repoRoot, 'themes/slingshot-lab');

describe('slingshot-lab theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('slingshot-lab');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('forked-slingshot');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"slingshot-lab\"]");
    expect(theme.css).toContain('--batch-theme-signature: "slingshot-lab"');
    expect(theme.css).toContain('--act-asset-forked-slingshot');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('04-angry-birds.png');
    expect(notice).toContain('04-angry-birds-standard-codex-imagegen-v1.png');
  });
});
