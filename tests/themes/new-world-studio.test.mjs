...
const themeRoot = join(repoRoot, 'themes/new-world-studio');

describe('new-world-studio theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('new-world-studio');
    expect(theme.manifest.version).toBe('0.1.1');
    expect(theme.manifest.mode).toBe('light');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('tidal-contour');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"new-world-studio\"]");
    expect(theme.css).toContain('--batch-theme-signature: "new-world-studio"');
    expect(theme.css).toContain('--act-asset-tidal-contour');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('12-one-piece-women.png');
    expect(notice).toContain('12-one-piece-women-standard-codex-imagegen-v1.png');
  });
});
