...
const themeRoot = join(repoRoot, 'themes/overworld-realms');

describe('overworld-realms theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('overworld-realms');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('voxel-cube');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"overworld-realms\"]");
    expect(theme.css).toContain('--batch-theme-signature: "overworld-realms"');
    expect(theme.css).toContain('--act-asset-voxel-cube');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('08-minecraft-realms.png');
    expect(notice).toContain('08-minecraft-standard-codex-imagegen-v1.png');
  });
});
