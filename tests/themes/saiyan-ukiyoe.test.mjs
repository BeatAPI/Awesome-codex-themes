...
const themeRoot = join(repoRoot, 'themes/saiyan-ukiyoe');

describe('saiyan-ukiyoe theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('saiyan-ukiyoe');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('brush-sun');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"saiyan-ukiyoe\"]");
    expect(theme.css).toContain('--batch-theme-signature: "saiyan-ukiyoe"');
    expect(theme.css).toContain('--act-asset-brush-sun');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('05-son-goku-ukiyoe.png');
    expect(notice).toContain('05-goku-standard-codex-imagegen-v1.png');
  });
});
