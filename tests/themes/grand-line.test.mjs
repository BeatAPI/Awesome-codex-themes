...
const themeRoot = join(repoRoot, 'themes/grand-line');

describe('grand-line theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('grand-line');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('compass-wedge');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"grand-line\"]");
    expect(theme.css).toContain('--batch-theme-signature: "grand-line"');
    expect(theme.css).toContain('--act-asset-compass-wedge');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('06-luffy.png');
    expect(notice).toContain('06-luffy-standard-codex-imagegen-v1.png');
  });
});
