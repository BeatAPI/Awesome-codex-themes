...
const themeRoot = join(repoRoot, 'themes/zaun-workshop');

describe('zaun-workshop theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('zaun-workshop');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('fractured-hex-mark');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"zaun-workshop\"]");
    expect(theme.css).toContain('--batch-theme-signature: "zaun-workshop"');
    expect(theme.css).toContain('--act-asset-fractured-hex-mark');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('02-jinx-arcane.png');
    expect(notice).toContain('02-jinx-standard-codex-imagegen-v2.png');
  });
});
