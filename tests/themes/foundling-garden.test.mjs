...
const themeRoot = join(repoRoot, 'themes/foundling-garden');

describe('foundling-garden theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('foundling-garden');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('seed-compass');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"foundling-garden\"]");
    expect(theme.css).toContain('--batch-theme-signature: "foundling-garden"');
    expect(theme.css).toContain('--act-asset-seed-compass');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('11-grogu-official.png');
    expect(notice).toContain('11-grogu-standard-codex-imagegen-v1.png');
  });
});
