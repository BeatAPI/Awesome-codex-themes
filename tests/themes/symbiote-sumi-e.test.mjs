...
const themeRoot = join(repoRoot, 'themes/symbiote-sumi-e');

describe('symbiote-sumi-e theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('symbiote-sumi-e');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('ink-drop-mon');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"symbiote-sumi-e\"]");
    expect(theme.css).toContain('--batch-theme-signature: "symbiote-sumi-e"');
    expect(theme.css).toContain('--act-asset-ink-drop-mon');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('03-venom-sumi-e.png');
    expect(notice).toContain('03-venom-standard-codex-imagegen-v1.png');
  });
});
