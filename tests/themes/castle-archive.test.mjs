...
const themeRoot = join(repoRoot, 'themes/castle-archive');

describe('castle-archive theme package', () => {
  test('loads as a complete experimental Schema 2 package', async () => {
    ...
    expect(theme.manifest.slug).toBe('castle-archive');
  ...
  test('declares a bounded original component-asset system', async () => {
    ...
    expect(theme.manifest.files.assets).toHaveProperty('keyhole-index');
  ...
  test('scopes visual rules to this theme and excludes the Satoru Gojo visual language', async () => {
    const theme = await loadThemePackage(themeRoot);
    expect(theme.css).toContain("html.awesome-codex-theme[data-awesome-codex-theme=\"castle-archive\"]");
    expect(theme.css).toContain('--batch-theme-signature: "castle-archive"');
    expect(theme.css).toContain('--act-asset-keyhole-index');
  ...
  test('records project asset sources without an internal-only restriction', async () => {
    ...
    expect(notice).toContain('07-lady-dimitrescu.png');
    expect(notice).toContain('07-lady-dimitrescu-standard-codex-imagegen-v1.png');
  });
});
